...
public class Acerca_miController {
    ...
    @GetMapping("/ver/{id}")
    @ResponseBody
    public Optional<Acerca_mi> findById(Long id){
        return acerca_miService.fndById(id);
    }
    
    @PostMapping("/new/about")
    public void save(@RequestBody Acerca_mi nuevoAcerca_mi){
        acerca_miService.save(nuevoAcerca_mi);        
    }
    
    @PostMapping("/edit/about/{id}")
    @ResponseBody
    public ResponseEntity<?> editAbout (@PathVariable("id") Long id, @RequestBody Acerca_miDto acerca_miDto){
        try {
